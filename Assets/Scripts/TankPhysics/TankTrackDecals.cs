...
public class TankTrackDecals : MonoBehaviour
{
    ...
    private void SpawnDecal(Vector3 rayOrigin, Vector3 rayDir, Vector3 forwardDir)
    {
        if (Physics.Raycast(rayOrigin, rayDir, out RaycastHit hit, raycastDistance, groundLayer))
        {
            ...
            decal.transform.SetPositionAndRotation(hit.point + hit.normal * 0.02f, Quaternion.LookRotation(-hit.normal, forwardDir));
            poolIndex++;
            if (poolIndex >= maxDecals) { poolIndex = 0; }
        }
    }
    private void OnDestroy() { if (poolContainer != null) { Destroy(poolContainer); } }
}