...
namespace GameScripts.Visuals
{
    public class TankTemperatureVisuals : MonoBehaviour
    {
        ...
        [Tooltip("Имя переменной Reference в твоем Shader Graph, отвечающей за базовый цвет танка.")]
        public string colorPropertyName = "_TemperatureColor";

        [Tooltip("Запасной вариант (для стандартных материалов Unity)")]
        ...
        private void Update()
        {
            ...
            if (useDebugTemperature)
            {
                targetTemp = debugTemperature;
            }
            else if (brain != null)
            {
                targetTemp = brain.networkTemperature.Value;
            }
            ...
            if (_displayTemp > 0)
            {
                targetColor = Color.Lerp(normalColor, fireColor, _displayTemp / 100f);
            }
            else if (_displayTemp < 0)
            {
                targetColor = Color.Lerp(normalColor, freezeColor, Mathf.Abs(_displayTemp) / 100f);
            }
            ...
            foreach (var rend in _renderers)
            {
                ...
                // Передаем цвет в шейдер по его правильному системному имени
