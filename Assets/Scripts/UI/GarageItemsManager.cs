...
public enum ItemCategory { Turret, Hull, Paint, Supply, Special }
...
public class GarageItemsManager : MonoBehaviour
{
    ...
    private void Start() { currentPlayerRankIndex = 0; LoadInventoryFromPlayFab(); }
    private void ApplyDefaultItemsLocally()
    {
        foreach (var item in itemsDatabase)
        {
            ...
            { item.isOwned = true; item.isEquipped = true; }
            else { item.isOwned = false; item.isEquipped = false; }
        }
    }
        ...
        PlayFabClientAPI.GetUserData(new GetUserDataRequest(),
            result =>
            {
                ...
                if (result.Data.ContainsKey("XP"))
                {
                    ...
                    { if (currentXp >= rankXpThresholds[i]) { currentPlayerRankIndex = i; break; } }
                }
                else { currentPlayerRankIndex = 0; }
                ...
                foreach (var item in itemsDatabase)
                {
                    if (item.itemID == defaultTurretID || item.itemID == defaultHullID || item.itemID == defaultPaintID)
                    { item.isOwned = true; }
                    else { item.isOwned = result.Data.ContainsKey("Owned_" + item.itemID); }
    ...
    private void SaveEquippedItemToCloud(GarageItemInfo item)
    {
        ...
        var request = new UpdateUserDataRequest { Data = new Dictionary<string, string> { { key, item.itemID } } };
    ...
    private void UpdateEquippedCache()
    {
        ...
        foreach (var item in itemsDatabase)
        {
            if (item.isEquipped)
            {
                ...
                { equippedHullPrefab = item.item3DModel; TankSetupData.SelectedHullID = item.itemID; }
                else if (item.category == ItemCategory.Turret)
                { equippedTurretPrefab = item.item3DModel; TankSetupData.SelectedTurretID = item.itemID; }
                else if (item.category == ItemCategory.Paint)
                { equippedSkinId = item.paintSkinId; TankSetupData.SelectedSkinID = item.paintSkinId; }
            }
        }
    }
    ...
    private Transform FindMountPoint(Transform parent)
    {
        ...
        foreach (Transform t in children) { if (t.name.ToLower() == "mount") return t; }
        return null;
    }
    ...
    private void RebuildTankPreview(GarageItemInfo previewItem)
    {
        ...
        { GameObject anchor = GameObject.Find("TankAnchor"); if (anchor != null) hullSpawnPoint = anchor.transform; }
        if (previewHull != null)
        { previewHull.SetActive(false); Destroy(previewHull); }
        if (hullSpawnPoint == null) return;
        ...
        if (hullToSpawn != null && hullSpawnPoint != null)
        {
            ...
            previewHull.transform.SetLocalPositionAndRotation(new Vector3(0f, tankYOffset, 0f), Quaternion.identity);
            ...
            if (mount != null && turretToSpawn != null)
            {
                ...
                previewTurret.transform.SetLocalPositionAndRotation(Vector3.zero, Quaternion.identity);
            }
    ...
    private void ApplySkinToPreview(string skinId)
    {
        ...
        if (previewHull != null)
        { foreach (Renderer r in previewHull.GetComponentsInChildren<Renderer>(true)) { ApplyToRenderer(r, config); } }
        if (previewTurret != null)
        { foreach (Renderer r in previewTurret.GetComponentsInChildren<Renderer>(true)) { ApplyToRenderer(r, config); } }
    }
    ...
    private void ApplyToRenderer(Renderer rend, TankSkinConfig config)
    {
        ...
        { Vector3 boundsSize = rend.GetComponent<MeshFilter>().sharedMesh.bounds.size; size = Mathf.Max(boundsSize.x, boundsSize.z); }
        else if (rend is SkinnedMeshRenderer smr && smr.sharedMesh != null)
        { Vector3 boundsSize = smr.sharedMesh.bounds.size; size = Mathf.Max(boundsSize.x, boundsSize.z); }
        else { size = Mathf.Max(rend.bounds.size.x, rend.bounds.size.z); }
        if (size <= 0.01f) size = 5f;
        Vector2 tiling = new Vector2(config.baseTiling * size, config.baseTiling * size);
        foreach (Material mat in rend.materials) { UpdateMaterial(mat, config, tiling); }
    }
    ...
    {
        if (isUIOpen) { if (selectedItemData != null) RebuildTankPreview(selectedItemData); }
        else { RebuildTankPreview(null); }
    }
    ...
    public void PopulateGarage(bool autoPreview = true)
    {
        ...
        foreach (var item in itemsDatabase) { if (item.category == currentCategory) { itemsToDisplay.Add(item); } }
        ...
        foreach (var item in itemsToDisplay)
        {
            ...
            if (cardScript != null)
            {
                ...
                if (firstCardInList == null) { firstCardInList = cardScript; firstItemData = item; }
                if (item.itemID == previouslySelectedID) { cardToSelect = cardScript; dataToSelect = item; }
                cardScript.OnCardClicked.AddListener(() => OnCardSelected(cardScript, item));
            }
        }
        if (cardToSelect == null && firstCardInList != null) { cardToSelect = firstCardInList; dataToSelect = firstItemData; }
        if (cardToSelect != null) { OnCardSelected(cardToSelect, dataToSelect, autoPreview); }
    ...
    private void UpdateRightPanel(GarageItemInfo item)
    {
        ...
        if (buyButton != null) { buyButton.gameObject.SetActive(!item.isOwned); buyButton.interactable = !isLockedByRank; }
        if (equipButton != null) { equipButton.gameObject.SetActive(item.isOwned); equipButton.interactable = !item.isEquipped; }
    }
    ...
    public void OnBuyButtonClicked()
    {
        ...
        if (uiManager != null)
        {
            if (uiManager.TrySpendCrystals(selectedItemData.price))
            {
                ...
                { Data = new Dictionary<string, string> { { "Owned_" + selectedItemData.itemID, "1" } } };
                PlayFabClientAPI.UpdateUserData(request,
    ...
    public void OnEquipButtonClicked()
    {
        ...
        foreach (var item in itemsDatabase)
        { if (item.category == selectedItemData.category) { item.isEquipped = false; } }
